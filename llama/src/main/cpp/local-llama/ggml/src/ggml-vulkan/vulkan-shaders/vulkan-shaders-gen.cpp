...
void execute_command(std::vector<std::string>& command, std::string& stdout_str, std::string& stderr_str) {
    ...
    // IMPORTANT: Close the write handles in the parent process so that ReadFile blocks correctly
    // and returns when the child process closes its end.
    ...
    std::thread stderr_thread([&]() {
        ...
        DWORD err_bytes;
        while (ReadFile(stderr_read, err_buffer.data(), (DWORD)err_buffer.size(), &err_bytes, NULL) && err_bytes > 0) {
            stderr_str.append(err_buffer.data(), err_bytes);
        }
    });
...
compile_count_guard acquire_compile_slot() {
    ...
    uint32_t N = std::max(1u, std::min(8u, std::thread::hardware_concurrency()));
...
void string_to_spv(std::string name, const std::string& source, const std::map<std::string, std::string>& defines, bool fp16 = true, bool coopmat = false, bool coopmat2 = false, bool f16acc = false) {
    ...
    // Always register in shader_fnames so the header is complete
    {
        std::lock_guard<std::mutex> guard(lock);
        shader_fnames.push_back(std::make_pair(name, out_path));
    }

    // Only compile if the source matches the filtered input
    if (basename(input_filepath) != source) {
        return;
    }
...
void matmul_shaders(bool fp16, MatMulIdType matmul_id_type, bool coopmat, bool coopmat2, bool f16acc) {
    ...
    if (input_filepath != "" && basename(input_filepath) != source_name) {
        return;
    }
    ...
    std::string suffix = std::string(f16acc ? "_f16acc" : "") + (coopmat ? "_cm1" : "") + (coopmat2 ? "_cm2" : (fp16 ? "" : "_fp32"));

    // Shaders with f16 B_TYPE
    string_to_spv(shader_name + "_f32_f16" + suffix,         source_name, merge_maps(merge_maps(base_dict, float_type_dict_f16), {{"DATA_A_F32", "1"},                                                     {"B_TYPE", "float16_t"},        {"D_TYPE", "float"}, }), fp16, coopmat, coopmat2, f16acc);
    string_to_spv(shader_name + "_f32_f16_aligned" + suffix, source_name, merge_maps(merge_maps(base_dict, float_type_dict_f16), {{"DATA_A_F32", "1"}, {"LOAD_VEC_A", load_vec}, {"LOAD_VEC_B", load_vec}, {"B_TYPE", aligned_b_type_f16}, {"D_TYPE", "float"}, {"ALIGNED", "1"}}), fp16, coopmat, coopmat2, f16acc);

    string_to_spv(shader_name + "_f16" + suffix,             source_name, merge_maps(merge_maps(base_dict, float_type_dict_f16), {{"DATA_A_F16", "1"},                                                     {"B_TYPE", "float16_t"},        {"D_TYPE", "float"}}), fp16, coopmat, coopmat2, f16acc);
    string_to_spv(shader_name + "_f16_aligned" + suffix,     source_name, merge_maps(merge_maps(base_dict, float_type_dict_f16), {{"DATA_A_F16", "1"}, {"LOAD_VEC_A", load_vec}, {"LOAD_VEC_B", load_vec}, {"B_TYPE", aligned_b_type_f16}, {"D_TYPE", "float"}, {"ALIGNED", "1"}}), fp16, coopmat, coopmat2, f16acc);
...
#endif
        {
            string_to_spv(shader_name + "_bf16" + suffix,         source_name, merge_maps(merge_maps(base_dict, float_type_dict_bf16), {{"TO_FLOAT_TYPE", to_float_type}, {"DATA_A_BF16", "1"},                             {"B_TYPE", coopmat2 ? "bfloat16_t" : "uint16_t"}, {"D_TYPE", "float"}, {"B_IS_FLOAT", "1"}, {"DATA_B_BF16", "1"}}),                   fp16, coopmat, coopmat2, f16acc);
            string_to_spv(shader_name + "_bf16_aligned" + suffix, source_name, merge_maps(merge_maps(base_dict, float_type_dict_bf16), {{"TO_FLOAT_TYPE", to_float_type}, {"DATA_A_BF16", "1"}, {"LOAD_VEC_A", load_vec_a}, {"LOAD_VEC_B", "4"}, {"B_TYPE", coopmat2 ? "bfloat16_t" : "u16vec4"},  {"D_TYPE", "float"}, {"B_IS_FLOAT", "1"}, {"DATA_B_BF16", "1"}, {"ALIGNED", "1"}}), fp16, coopmat, coopmat2, f16acc);
        }
    }

    for (const auto& tname : type_names) {
        std::string load_vec_quant = "2";
        if ((tname == "q4_0") || (tname == "q4_1") || (tname == "q5_1") || (tname == "iq1_s") || (tname == "iq1_m") || (tname == "iq2_xxs") || (tname == "iq2_xs") || (tname == "iq2_s") || (tname == "turbo4_0"))
            load_vec_quant = "8";
        ...
        if (!coopmat2) {
            string_to_spv(shader_name + "_" + tname + "_f32" + suffix + (fp16 ? "" : "_fp32"),         source_name, merge_maps(merge_maps(base_dict, float_type_dict), {{data_a_key, "1"}, {"LOAD_VEC_A", load_vec_a_unaligned},                           {"B_TYPE", "float"},            {"D_TYPE", "float"}}), fp16, coopmat, coopmat2, f16acc);
            string_to_spv(shader_name + "_" + tname + "_f32_aligned" + suffix + (fp16 ? "" : "_fp32"), source_name, merge_maps(merge_maps(base_dict, float_type_dict), {{data_a_key, "1"}, {"LOAD_VEC_A", load_vec_a},           {"LOAD_VEC_B", load_vec}, {"B_TYPE", aligned_b_type_f32}, {"D_TYPE", "float"}, {"ALIGNED", "1"}}), fp16, coopmat, coopmat2, f16acc);
        }

        if (tname != "f16" && tname != "f32") {
            string_to_spv(shader_name + "_" + tname + "_f16" + suffix + (fp16 ? "" : "_fp32"),         source_name,  merge_maps(merge_maps(base_dict, float_type_dict), {{data_a_key, "1"}, {"LOAD_VEC_A", load_vec_a_unaligned},                           {"B_TYPE", "float16_t"},        {"D_TYPE", "float"}}), fp16, coopmat, coopmat2, f16acc);
            string_to_spv(shader_name + "_" + tname + "_f16_aligned" + suffix + (fp16 ? "" : "_fp32"), source_name,  merge_maps(merge_maps(base_dict, float_type_dict), {{data_a_key, "1"}, {"LOAD_VEC_A", load_vec_a},           {"LOAD_VEC_B", load_vec}, {"B_TYPE", aligned_b_type_f16}, {"D_TYPE", "float"}, {"ALIGNED", "1"}}), fp16, coopmat, coopmat2, f16acc);
        }

#if defined(GGML_VULKAN_INTEGER_DOT_GLSLC_SUPPORT)
        // Integer dot mmq performs better with f32 accumulators
        if (!f16acc && !coopmat && !coopmat2 && (is_legacy_quant(tname) || is_k_quant(tname) || tname == "mxfp4")) {
            std::string s_name_q81 = shader_name + "_" + tname + "_q8_1" + (fp16 ? "" : "_fp32");
            string_to_spv(s_name_q81, "mul_mmq.comp", merge_maps(merge_maps(base_dict, float_type_dict), {{data_a_key, "1"}, {"D_TYPE", "float"},}), fp16, coopmat, coopmat2, f16acc);
        }
#endif
    }
}
...
void process_shaders() {
    ...
    for (const bool& fp16 : {false, true}) {
        ...
        // flash attention
        {
            ...
            for (const auto& tname : type_names) {
                ...
                std::string block_byte_size = "0";
                if (tname == "f32") block_byte_size = "16";
                else if (tname == "f16") block_byte_size = "2";
                else if (tname == "q4_0") block_byte_size = "18";
                else if (tname == "q4_1") block_byte_size = "20";
                ...
                fa_base_dict["A_TYPE"] = (tname == "f16") ? "float16_t" : "float";
                fa_base_dict["A_TYPE_VEC4"] = (tname == "f16") ? "f16vec4" : "vec4";
                if (fp16) {
#if defined(GGML_VULKAN_COOPMAT2_GLSLC_SUPPORT)
                if (tname == "f16") {
                    string_to_spv("flash_attn_f32_f16_" + tname + fa_suffix, "flash_attn_cm2.comp",
                        merge_maps(fa_base_dict, {{"DATA_A_F16", "1"}, {"Q_TYPE", "float"}, {"D_TYPE", "float"}, {"D_TYPEV4", "vec4"}, {"BLOCK_BYTE_SIZE", block_byte_size}}), fp16, false, true, f16acc);
                } else {
                    std::string data_a_key = "DATA_A_" + to_uppercase(tname);
                    string_to_spv("flash_attn_f32_f16_" + tname + fa_suffix, "flash_attn_cm2.comp",
                        merge_maps(fa_base_dict, {{data_a_key, "1"}, {"Q_TYPE", "float"}, {"D_TYPE", "float"}, {"D_TYPEV4", "vec4"}, {"DEQUANTFUNC", "dequantFunc"+to_uppercase(tname) }, {"BLOCK_SIZE", "QUANT_K_"+to_uppercase(tname) }, {"BLOCK_BYTE_SIZE", block_byte_size}}), fp16, false, true, f16acc);
                }
#endif
#if defined(GGML_VULKAN_COOPMAT_GLSLC_SUPPORT)
                if (tname == "f16") {
                    string_to_spv("flash_attn_f32_f16_" + tname + fa_suffix, "flash_attn_cm1.comp",
                        merge_maps(fa_base_dict, {{"DATA_A_F16", "1"}, {"Q_TYPE", "float"}, {"D_TYPE", "float"}, {"D_TYPEV4", "vec4"}, {"COOPMAT", "1"}, {"BLOCK_BYTE_SIZE", block_byte_size}}), fp16, true, false, f16acc);
                } else if (tname == "q4_0" || tname == "q8_0" || tname == "f32" || tname == "turbo4_0") {
                    std::string data_a_key = "DATA_A_" + to_uppercase(tname);
                    string_to_spv("flash_attn_f32_f16_" + tname + fa_suffix, "flash_attn_cm1.comp",
                        merge_maps(fa_base_dict, {{data_a_key, "1"}, {"Q_TYPE", "float"}, {"D_TYPE", "float"}, {"D_TYPEV4", "vec4"}, {"BLOCK_SIZE", "QUANT_K_"+to_uppercase(tname)}, {"COOPMAT", "1"}, {"BLOCK_BYTE_SIZE", block_byte_size}}), fp16, true, false, f16acc);
                }
#endif
                }

                if (tname == "f16") {
                    string_to_spv("flash_attn_f32_f16_" + tname + fa_suffix, "flash_attn.comp",
                        merge_maps(fa_base_dict, {{"DATA_A_F16", "1"}, {"Q_TYPE", "float"}, {"D_TYPE", "float"}, {"D_TYPEV4", "vec4"}, {"BLOCK_BYTE_SIZE", block_byte_size}}), fp16, false, false, f16acc);
                } else {
                    std::string data_a_key = "DATA_A_" + to_uppercase(tname);
                    string_to_spv("flash_attn_f32_f16_" + tname + fa_suffix, "flash_attn.comp",
                        merge_maps(fa_base_dict, {{data_a_key, "1"}, {"Q_TYPE", "float"}, {"D_TYPE", "float"}, {"D_TYPEV4", "vec4"}, {"BLOCK_SIZE", "QUANT_K_"+to_uppercase(tname) }, {"BLOCK_BYTE_SIZE", block_byte_size}}), fp16, false, false, f16acc);
                }
            }
        }
    }
    ...
    // turbo4_0 copy shaders
    string_to_spv("cpy_f32_turbo4_0", "copy_to_quant.comp", {{"DATA_A_TURBO4_0", "1"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}});
    string_to_spv("cpy_turbo4_0_f32", "copy_from_quant.comp", {{"DATA_A_TURBO4_0", "1"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}});

    for (std::string t : {"f32", "f16", "bf16", "q4_0", "q4_1", "q5_0", "q5_1", "q8_0", "iq4_nl", "turbo4_0"}) {
