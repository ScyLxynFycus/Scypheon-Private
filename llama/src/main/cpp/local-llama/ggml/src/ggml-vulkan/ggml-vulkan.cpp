...
static bool g_is_mali_stability_active = false;
...
#define VK_CHECK(err, msg)                                          \
    do {                                                            \
        ...
        if (err_ != vk::Result::eSuccess) {                         \
            ...
            throw std::runtime_error("ggml_vulkan: " + std::string(#err) + " failed"); \
        }                                                           \
    } while (0)
...
#define CREATE_MM2(TYPE, PIPELINE_NAME, NAMELC, WG_DENOMS, WARPTILE, PUSHCONST, PARAMCOUNT, ID, REQSUBGROUPSIZE) \
        ...
        CREATE_MM2(GGML_TYPE_TURBO4_0, pipeline_dequant_mul_mat_mat[GGML_TYPE_TURBO4_0], matmul_turbo4_0_f32, mmq_wg_denoms, warptile_mmq, vk_mat_mat_push_constants, 3, , 0);

#if 0
#if defined(GGML_VULKAN_INTEGER_DOT_GLSLC_SUPPORT)
        if (device->integer_dot_product) {
            // ... (missing q8_1 symbols)
        }
#endif
#endif
        ...
        if (device->subgroup_ballot && device->subgroup_require_full_support && subgroup_min_size_16) {
            ...
            CREATE_MM2(GGML_TYPE_TURBO4_0, pipeline_dequant_mul_mat_mat_id[GGML_TYPE_TURBO4_0], matmul_id_subgroup_turbo4_0_f32, mmq_wg_denoms, warptile_mmqid, vk_mat_mat_id_push_constants, mul_mat_id_param_count, _id, mul_mat_subgroup_size);
...
#undef CREATE_MM
    ...
    if (device->float_controls_rte_fp16) {
        ...
        ggml_vk_create_pipeline(device, device->pipeline_cpy_f32_quant[GGML_TYPE_TURBO4_0], "cpy_f32_turbo4_0", cpy_turbo4_0_f32_len, cpy_turbo4_0_f32_data, "main", 2, sizeof(vk_op_unary_push_constants), {32, 1, 1}, {}, 1);
    } else {
...
#define SET_ROWS(itype, rte) \
        ...
        ggml_vk_create_pipeline(device, device->pipeline_set_rows ## itype [GGML_TYPE_TURBO4_0], "set_rows_turbo4_0" #itype, set_rows_turbo4_0 ## itype ## rte ## _len, set_rows_turbo4_0 ## itype ## rte ## _data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true);
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        // 🛡️ Mali Stability Profile Activation
        if (strstr(device->name.c_str(), "Mali-G77") || strstr(device->name.c_str(), "Mali-G78")) {
            g_is_mali_stability_active = true;
            device->uma = true; // Force UMA path for Mali
            VK_LOG_DEBUG("🛡️ Mali Stability Profile Active: Applying conservative sync flags.");
        }
...
ggml_backend_t ggml_backend_vk_init(size_t dev_num) {
    const char * env_vk = getenv("GGML_VULKAN");
    if (env_vk && strcmp(env_vk, "0") == 0) {
        return NULL;
    }
    if (g_vulkan_disabled) {
        return nullptr;
    }
...
extern "C" bool g_vulkan_disabled;
