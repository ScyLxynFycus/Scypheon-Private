...
layout (binding = 1) readonly buffer K_PACKED {A_TYPE_VEC4 k_data_packed[];} k_packed;
layout (binding = 2) readonly buffer V_PACKED {A_TYPE_VEC4 v_data_packed[];} v_packed;
...
#if defined(DATA_A_F16) || defined(DATA_A_BF16)
#ifndef BLOCK_BYTE_SIZE
...
FLOAT_TYPEV4 dequantize4(uint ib, uint iqs, uint a_offset, uint binding_idx) {
    ...
    if (binding_idx == BINDING_IDX_K) {
        ...
        d = float(k_packed.k_data_packed16[a_offset + ib].d);
    } else {
        qs_val = uint(v_packed.v_data_packed16[a_offset + ib].qs[iqs / 2]);
        d = float(v_packed.v_data_packed16[a_offset + ib].d);
    }
    // Each 16-bit packed short contains 4 nibbles (indices iqs, iqs+1, iqs+2, iqs+3)
    // Actually, if we use iqs/2, we get indices iqs and iqs+1? No.
    // If qs is uint16_t, each element has 4 nibbles. So qs[0] has elements 0,1,2,3.
    // iqs is element index. So we want qs[iqs/4]?
    // Wait, let's re-verify: indices[0..3] in qs[0].
    // So if iqs = 0, we want qs[0].
    // If iqs = 4, we want qs[1].
    // Thus index should be iqs / 4.
    uint val;
    if (binding_idx == BINDING_IDX_K) {
        val = uint(k_packed.k_data_packed16[a_offset + ib].qs[iqs / 4]);
    } else {
        val = uint(v_packed.v_data_packed16[a_offset + ib].qs[iqs / 4]);
    }

    return FLOAT_TYPEV4(
        FLOAT_TYPE(d) * FLOAT_TYPE(T4C[val & 0xF]),
        FLOAT_TYPE(d) * FLOAT_TYPE(T4C[(val >> 4) & 0xF]),
        FLOAT_TYPE(d) * FLOAT_TYPE(T4C[(val >> 8) & 0xF]),
        FLOAT_TYPE(d) * FLOAT_TYPE(T4C[val >> 12])
    );
}
#endif
