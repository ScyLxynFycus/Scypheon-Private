...
// TurboFlash: two-pass fused asymmetric attention for single-token V decode (Victory/TURBO4_0)
// Returns true when V is TURBO4_0 and logic conditions for single-token decode are met
static bool ggml_metal_op_flash_attn_ext_use_turbo_flash(const ggml_tensor * op) {
    ...
    // Only for TURBO4_0 V cache
    if (type_v != GGML_TYPE_TURBO4_0) return false;

    // Only for q8_0 or TURBO4_0 K — asymmetric or symmetric turbo
    if (type_k != GGML_TYPE_Q8_0 && type_k != GGML_TYPE_TURBO4_0) return false;
...
int ggml_metal_op_flash_attn_ext(ggml_metal_op_t ctx, int idx) {
    ...
    // Allow asymmetric K/V quantization for supported mixed pairs
    {
        ...
        const ggml_type type_v = op->src[2]->type;
        if (type_k != type_v) {
            const bool k_is_turbo = (type_k == GGML_TYPE_TURBO4_0);
    ...
    //   - V is TURBO4_0, K is q8_0 or TURBO4_0
    ...
    if (ggml_metal_op_flash_attn_ext_use_turbo_flash(op)) {
        ...
        // so turbo data starts at:
        {
            ...
            // ---- Pass 1: Block scoring + partial V accumulation ----
            {
                ...
                const ggml_type type_k = op->src[1]->type;
                const bool k_is_turbo4 = (type_k == GGML_TYPE_TURBO4_0);

                char p1_base[128];
                char p1_name[256];
                snprintf(p1_base, 128, "kernel_turbo_flash_p1_dk%d_dv%d", dk, dv);
                snprintf(p1_name, 256, "%s_mask=%d_dk=%d_dv=%d_kt4=%d",
                        p1_base, has_mask ? 1 : 0, dk, dv, k_is_turbo4 ? 1 : 0);
                ...
                if (!res_p1.pipeline) {
                    ...
                    ggml_metal_cv_set_bool(cv,  k_is_turbo4,  FC_TURBO_FLASH_P1 + 3);
                ...
                ggml_metal_encoder_set_buffer  (enc, bid_src2, 3);  // v (TURBO4_0)
