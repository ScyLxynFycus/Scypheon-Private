...
// All legacy turbo2/3 templates purged. Only standard quants and TURBO4_0 remain.
...
constant bool    FC_turbo_flash_p1_k_is_turbo4 [[function_constant(FC_TURBO_FLASH_P1 + 3)]];
...
kernel void kernel_turbo_flash_p1(
    ...
    // ====== Load TURBO4_0 codebook into registers (16 entries) ======
    float v_cb[16];
    for (int i = 0; i < 16; i++) {
        v_cb[i] = float(turbo_centroids_4bit_h[i]);
    }

    // K codebook — only loaded when K is TURBO4_0
    float k_cb[16];
    if (FC_turbo_flash_p1_k_is_turbo4) {
        for (int i = 0; i < 16; i++) {
            k_cb[i] = float(turbo_centroids_4bit_h[i]);
        }
    }
    ...
    for (int t = t_start; t < t_end; t++) {
        ...
        if (FC_turbo_flash_p1_k_is_turbo4) {
            // K is TURBO4_0: norm, qs[64]
            device const block_turbo4_0 * k_row = (device const block_turbo4_0 *)(k_base + t * args.nb11);
            ...
            for (short i = 0; i < DK_PER_LANE; i++) {
                ...
                const int qs_byte = d / 2;
                const int qs_shift = (d % 2) * 4;
                const uint8_t centroid_idx = (k_row[0].qs[qs_byte] >> qs_shift) & 0x0F;
        ...
        // V is TURBO4_0: one block_turbo4_0 per 128 dims (QK_TURBO4=128).
        device const block_turbo4_0 * v_row = (device const block_turbo4_0 *)(v_base + t * args.nb21);
        ...
        for (short i = 0; i < DV_PER_LANE; i++) {
            ...
            const int qs_byte = d / 2;
            const int qs_shift = (d % 2) * 4;
            const uint8_t centroid_idx = (v_row[0].qs[qs_byte] >> qs_shift) & 0x0F;
