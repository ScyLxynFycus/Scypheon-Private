    ...
    for (uint32_t il = 0; il < hparams.n_layer; il++) {
        ...
        ggml_type layer_type_v = type_v;
        {
            static const int adaptive_mode = [&]() {
                ...
                // Auto-enable Boundary V (mode 7) when V is turbo4
                if (type_v == GGML_TYPE_TURBO4_0 && hparams.n_layer >= 8) {
                    LLAMA_LOG_INFO("llama_kv_cache: Boundary V auto-enabled for turbo4-V (opt-out: TURBO_LAYER_ADAPTIVE=0)\n");
                    return 7;
                }
                return 0;
            }();
            const bool is_turbo = (type_k == GGML_TYPE_TURBO4_0);
            const bool v_is_turbo = (type_v == GGML_TYPE_TURBO4_0);
            ...
            if (adaptive_mode == 1 && is_turbo && n_layer >= 8) {
                ...
                // Boundary V (turbo4 boundaries): first2+last2 V=turbo4, rest V=turbo2 (No, everything is turbo4 now)
                // Since turbo2 is gone, mode 5 is less useful, but we'll keep the structure for turbo4 vs something else? 
                // Actually, if everything is turbo4, we don't need these modes to fallback to deleted types.
                // I will update mode 5/6/7 to use turbo4 as the "rest".
                const bool is_boundary = (il < 2 || il >= n_layer - 2);
                layer_type_v = is_boundary ? GGML_TYPE_TURBO4_0 : GGML_TYPE_TURBO4_0; // rest was turbo2
                if (il == 0) {
                    LLAMA_LOG_INFO("llama_kv_cache: Boundary V mode 5: all V=turbo4 (experimental purge stage)\n");
                }
            } else if (adaptive_mode == 6 && v_is_turbo && n_layer >= 8) {
                // V-only: last 8 V=turbo4, rest V=turbo4 (was turbo2)
                layer_type_v = GGML_TYPE_TURBO4_0;
                if (il == 0) {
                    LLAMA_LOG_INFO("llama_kv_cache: V-only LA mode 6: all V=turbo4 (experimental purge stage)\n");
                }
            } else if (adaptive_mode == 7 && v_is_turbo && n_layer >= 8) {
                // Boundary V (recommended): first2+last2 V=q8_0, rest V=turbo4 (was turbo2)
                const bool is_boundary = (il < 2 || il >= n_layer - 2);
                layer_type_v = is_boundary ? GGML_TYPE_Q8_0 : GGML_TYPE_TURBO4_0;
                if (il == 0) {
                    LLAMA_LOG_INFO("llama_kv_cache: Boundary V mode 7: first2+last2 V=q8_0, rest V=turbo4\n");
                }
            }
        }
        // For turbo types, pad K head_dim to next multiple of 128 for full WHT groups
        uint32_t n_embd_k_gqa_eff = n_embd_k_gqa;
        const bool k_is_turbo = (layer_type_k == GGML_TYPE_TURBO4_0);
        ...
        const bool v_is_turbo = (layer_type_v == GGML_TYPE_TURBO4_0);
