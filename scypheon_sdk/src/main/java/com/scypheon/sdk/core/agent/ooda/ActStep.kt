...
class ActStep @Inject constructor(
    ...
    suspend fun execute(
        ...
        if (orientation.requiresDeepReasoning) {
            Timber.i("噫 [OODA_ACT] Delegating to ORIGA due to complexity.")
        ...
        Timber.d("噫 [OODA_ACT] Executing tool: ${decision.toolName} | Trace: $executionId")
        ...
        try {
            ...
            if (!validation.isValid) {
                Timber.w("笞・・[OODA_ACT] Output validation failed: ${validation.reason}")
                return@withContext FastPathResult(
                    skillName = orientation.selectedSkill.type.name,
                    toolName = decision.toolName,
            ...
            Timber.i("笨・[OODA_ACT] Executed & validated in ${elapsed}ms | Trace: $executionId")
            FastPathResult(
                skillName = orientation.selectedSkill.type.name,
                toolName = decision.toolName,
            ...
            val elapsed = SystemClock.elapsedRealtime() - startTime
            Timber.e(e, "笶・[OODA_ACT] Execution failed: ${decision.toolName} | Trace: $executionId")
