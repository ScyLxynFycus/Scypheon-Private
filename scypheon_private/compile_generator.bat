...
call "%VCVARS_PATH%"
cl /EHsc /O2 /std:c++17 /I. "%~dp0..\llama\src\main\cpp\local-llama\ggml\src\ggml-vulkan\vulkan-shaders\vulkan-shaders-gen.cpp" /Fe:"%~dp0vulkan-shaders-gen-new.exe"
if %ERRORLEVEL% NEQ 0 (
    echo Compilation failed.
    exit /b %ERRORLEVEL%
)

echo Compilation successful: vulkan-shaders-gen-new.exe
